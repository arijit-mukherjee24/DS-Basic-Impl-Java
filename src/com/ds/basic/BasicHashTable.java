...
public class BasicHashTable<X, Y> {
	...
	public Y delete(X key) {
		// first get the value for the key so it can be returned
		Y value = get(key);

		// clear out the hashtable slot for the key and return the removed table
		if (value != null) {
			int hash = calculateHash(key);
			data[hash] = null;
			size--;

			hash = (hash + 1) % this.capacity; // checking for other collision
			...
			while (data[hash] != null) {
				...
				// To check the rehashing concept in the motorcycle example
	...
	public boolean hasKey(X key) {
		...
		//otherwise get the hashentry for the given key and see if it matches the given key
		else {
	...
	public boolean hasValue(Y value) {
		// loop through all of the hash enttries
